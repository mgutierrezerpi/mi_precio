...
;(async () => {
  ...
  await ctx.addInitScript((t) => {
    ...
    localStorage.setItem(
      'auth_state',
      JSON.stringify({ user: t.user, tenant: t.tenant })
    )
  }, tok)
  const p = await ctx.newPage()
  for (const [name, url] of [
    ['dashboard_full', '/admin'],
    ['productos_full', '/admin/items'],
  ]) {
  ...
  await p2.goto(ORIGIN + '/p/' + tok.tenant.subdomain, {
  ...
  await autoscroll(p2)
  await p2.screenshot({
    path: path.join(OUT, 'publica_full.png'),
    fullPage: true,
  })
  ...
  await p3.goto(ORIGIN + '/p/' + tok.tenant.subdomain, {
    waitUntil: 'networkidle',
    timeout: 45000,
  })
  ...
  await autoscroll(p3)
  await p3.screenshot({
    path: path.join(OUT, 'publica_movil_full.png'),
    fullPage: true,
  })
