...
# Skip Apartment configuration during asset precompilation
if ENV['SECRET_KEY_BASE_DUMMY'] == '1' || 
   defined?(Rake) || 
   File.basename($0) == 'rake' ||
   ARGV.any? { |arg| arg.include?('assets:precompile') }
  
  Rails.logger.info "Skipping Apartment configuration during asset precompilation"
  return
end
...
Apartment.configure do |config|
  ...
  #
  config.tenant_names = -> { 
    # Skip database queries during asset precompilation
    return [] if Rails.env.test? || defined?(Rails::Console) || File.basename($0) == "rake" || defined?(Rake)
    
    begin
      Tenant.pluck(:database) 
...
# Custom elevator configuration is in config/initializers/apartment_elevator.rb
