...
class LinkTree(BaseModel):
    ...
    public_slug = CharField(max_length=63, unique=True, index=True)
    ...
    font = CharField(max_length=32, default="sans")
    tags = TextField(default="[]")
    links = TextField(default="[]")
    instagram_url = TextField(null=True)
    tiktok_url = TextField(null=True)
    email_url = TextField(null=True)
