...
def test_customer_aggregates_reflect_orders(tenant):
    ...
    customers.create_order(
        c.id, items=[{"name": "A", "quantity": 1, "unit_price": 100}]
    )
    customers.create_order(
        c.id, items=[{"name": "B", "quantity": 1, "unit_price": 250}]
    )
...
def test_list_orders_newest_first_with_items(tenant):
    ...
    customers.create_order(
        c.id, items=[{"name": "First", "quantity": 1, "unit_price": 10}]
    )
    customers.create_order(
        c.id, items=[{"name": "Second", "quantity": 1, "unit_price": 20}]
    )
...
def test_customer_stats(tenant):
    ...
    customers.create_order(
        recent.id, items=[{"name": "x", "quantity": 1, "unit_price": 10}]
    )
    ...
    for _ in range(3):
        customers.create_order(
            rec.id, items=[{"name": "x", "quantity": 1, "unit_price": 10}]
        )

    # An old customer with an old purchase (inactive).
    old = customers.create_customer(tenant.id, name="Viejo")
    o = customers.create_order(
        old.id, items=[{"name": "x", "quantity": 1, "unit_price": 10}]
    )
    Order.update(created_at=datetime.utcnow() - timedelta(days=60)).where(
        Order.id == o.id
    ).execute()
...
def test_update_order_replaces_items_and_recomputes_total(tenant):
    ...
    order = customers.create_order(
        c.id, items=[{"name": "A", "quantity": 1, "unit_price": 100}], reference="R-1"
    )
