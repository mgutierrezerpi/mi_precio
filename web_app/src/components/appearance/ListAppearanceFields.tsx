...
export const LIST_DESIGNS: ListDesign[] = ['store', 'classic', 'nordic', 'fine', 'modern', 'photo', 'cards', 'catalog', 'tech']

export const BRAND_SWATCHES = ['#7C3AED', '#2563EB', '#0EA5E9', '#10B981', '#F59E0B', '#EF4444', '#DB2777', '#475569']
...
export function hasOwnAppearance(a: Pick<ListAppearance, 'design' | 'heroColor' | 'bgUrl'>): boolean {
...
export function DesignThumb({ design, accent }: { design: ListDesign; accent: string }) {
  if (design === 'store') {
    return (
 ...
 */
export function ListAppearanceFields({ t, value, onChange, accent, inherited, disabled, savedDesign, savedBg }: Props) {
  const bgFileRef = useRef<HTMLInputElement>(null)
  const canInherit = !!inherited
  const effectiveDesign = value.design ?? inherited?.design ?? 'store'
  const effectiveHero = value.heroColor ?? (canInherit ? inherited?.heroColor ?? null : null)
  ...
  const pickBg = async (file?: File) => {
    ...
    onChange({ bgUrl: await fileToDataUrl(file, 1600), bgOverlay: value.bgOverlay ?? false })
  }
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-1.5">
        <span className="text-xs font-bold text-[var(--dash-text2)]">{t('set.hero.title')}</span>
        <span className="text-[11px] font-medium leading-snug text-[var(--dash-muted)]">{t('set.hero.subtitle')}</span>
        <div className="mt-1 flex flex-wrap items-center gap-2.5">
          {BRAND_SWATCHES.map((c) => (
            <button key={c} type="button" disabled={disabled} onClick={() => onChange({ heroColor: c })} title={c}
              ...
              style={{ backgroundColor: c, boxShadow: (value.heroColor ?? '').toUpperCase() === c ? `0 0 0 2px ${c}` : undefined }} />
          ))}
          <span className="mx-1 h-6 w-px bg-[var(--dash-border)]" />
          <input type="color" value={effectiveHero ?? accent} disabled={disabled} onChange={(e) => onChange({ heroColor: e.target.value.toUpperCase() })}
            className="h-8 w-10 cursor-pointer rounded border border-[var(--dash-border)] bg-transparent disabled:opacity-60" />
          <input value={effectiveHero ?? accent} disabled={disabled} onChange={(e) => onChange({ heroColor: e.target.value ? e.target.value.toUpperCase() : null })}
            className={`${inputCls} h-8 w-28 font-mono`} />
          {value.heroColor && !disabled && (
            <button type="button" onClick={() => onChange({ heroColor: null })} className="text-[12px] font-bold text-[var(--dash-link)] hover:underline">
              {canInherit ? t('list.appearance.useTenant') : t('set.hero.useBrand')}
            </button>
          )}
        </div>
      </div>
      ...
      <div className="mt-1 flex items-center justify-between gap-2">
        <div className="flex flex-col gap-0.5">
          ...
          <p className="text-xs font-medium text-[var(--dash-muted)]">{canInherit ? t('list.appearance.designSub') : t('set.design.subtitle')}</p>
        </div>
        {savedDesign && <span className="shrink-0 text-xs font-bold text-[var(--tone-green-fg)]">{t('common.saved')}</span>}
      </div>
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        {canInherit && (
          ...
          >
            <DesignThumb design={inherited?.design ?? 'store'} accent={accent} />
            <div className="flex items-start justify-between gap-2 border-t border-[var(--dash-border)] bg-[var(--dash-surface)] p-3">
              <div className="flex flex-col gap-0.5">
                <span className="text-[13px] font-bold text-[var(--dash-text)]">{t('list.appearance.inherit')}</span>
                <span className="text-[11px] font-medium leading-tight text-[var(--dash-muted)]">{t('list.appearance.inheritDesc')}</span>
              </div>
              {value.design === null && <Icon name="circle-check" size={16} className="mt-0.5 shrink-0 text-[#7C3AED]" />}
            </div>
          </button>
        )}
        {LIST_DESIGNS.map((d) => {
      ...
      <div className="mt-1 flex items-center justify-between gap-2">
        ...
        {savedBg && <span className="shrink-0 text-xs font-bold text-[var(--tone-green-fg)]">{t('common.saved')}</span>}
      </div>
      <div className="flex flex-col gap-4 rounded-2xl border border-[var(--dash-border)] p-4">
        <div className="flex items-center gap-4">
          <span className="relative flex h-16 w-24 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-[var(--dash-border)] bg-[var(--dash-soft)]">
            ...
            {bgUrl && bgOverlay && <span className="absolute inset-0" style={{ background: accent, opacity: 0.5, mixBlendMode: 'multiply' }} />}
          </span>
          {!disabled && (
            <div className="flex flex-col gap-1.5">
              <div className="flex items-center gap-2">
                <input ref={bgFileRef} type="file" accept="image/*" className="hidden" onChange={(e) => pickBg(e.target.files?.[0])} />
                <button type="button" onClick={() => bgFileRef.current?.click()} className="flex h-9 items-center gap-2 rounded-[10px] border border-[var(--dash-border)] bg-[var(--dash-surface)] px-3.5 text-[13px] font-bold text-[var(--dash-text2)] hover:bg-[var(--dash-soft)]">
                  <Icon name="upload" size={15} /> {bgUrl ? t('set.bg.change') : t('set.bg.upload')}
                </button>
                {value.bgUrl && (
                  <button type="button" onClick={() => onChange({ bgUrl: null, bgOverlay: null })} className="text-[13px] font-bold text-[#EF4444] hover:underline">
                    {canInherit ? t('list.appearance.useTenant') : t('common.remove')}
                  </button>
                )}
              </div>
              {bgIsInherited && bgUrl && <span className="text-[11px] font-semibold text-[var(--dash-muted)]">{t('list.appearance.bgInherited')}</span>}
            </div>
          )}
        </div>
        <div className="flex items-center justify-between gap-4 rounded-xl border border-[var(--dash-border)] p-3.5">
          ...
          <Toggle on={bgOverlay} disabled={disabled || !bgUrl || bgIsInherited} onClick={() => onChange({ bgOverlay: !bgOverlay })} />
        </div>
      </div>
    </>
  )
