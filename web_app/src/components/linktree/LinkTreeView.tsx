...
const WHATSAPP_ICON_PATH = [
  'M8.6 7.7c.17-.4.35-.4.52-.4h.45c.14 0 .33.05.42.33l.66 1.58c.07.18.05.34-.02.48',
  'l-.32.48c-.1.12-.2.25-.08.47.12.22.55.9 1.2 1.45.83.72 1.53.95 1.75 1.07',
  '.22.12.35.1.48-.06l.6-.7c.13-.16.28-.13.47-.07l1.55.73c.2.1.33.15.38.24',
  '.05.1.05.56-.13 1.1-.18.54-1.06 1.03-1.47 1.08-.38.05-.86.18-2.92-.63',
  '-2.5-.98-4.1-3.47-4.23-3.63-.12-.16-1-1.33-1-2.54 0-1.2.63-1.8.85-2.04Z',
].join('')
...
function LinkTreeIcon({ name, size = 20 }: { name: string; size?: number }) {
  ...
  const paths: Record<string, React.ReactNode> = {
    ...
    link: (
      <>
        <path d="M9 15 7.5 16.5a3.5 3.5 0 0 1-5-5L5 9" />
        <path d="m15 9 1.5-1.5a3.5 3.5 0 0 1 5 5L19 15" />
        <path d="m8 16 8-8" />
      </>
    ),
    map: (
    ...
    tiktok: (
      <path
        d="M14 3v10.3a4.3 4.3 0 1 1-3.6-4.25v3.05a1.5 1.5 0 1 0 .8 1.33V3h2.8c.25 2.25 1.55 3.65 3.7 3.9v2.7A7.4 7.4 0 0 1 14 8.55"
        fill="currentColor"
        stroke="none"
      />
    ),
    whatsapp: (
      <>
        <path d="M20.5 11.6a8.45 8.45 0 0 1-12.55 7.4L3.5 20.4l1.45-4.15A8.45 8.45 0 1 1 20.5 11.6Z" />
        <path
          d={WHATSAPP_ICON_PATH}
          fill="currentColor"
          stroke="none"
        />{' '}
      </>
...
export function LinkTreeView({
  ...
  const template =
    data.template === 'editorial' || data.template === 'atelier'
      ? data.template
      : 'botanical'
  const shareUrl = publicUrl || window.location.href
  const pageClassName = [
    `link-tree-page link-tree-page--${template} link-tree-page--font-${data.font || 'sans'}`,
    preview ? 'link-tree-page--preview' : '',
  ].join(' ')
  ...
  useEffect(() => {
    ...
    const previousBackground = root.style.getPropertyValue(
      '--linktree-background'
    )
    ...
    return () => {
      ...
      if (previousColor)
        root.style.setProperty('--linktree-scrollbar', previousColor)
      else root.style.removeProperty('--linktree-scrollbar')
      if (previousBackground)
        root.style.setProperty('--linktree-background', previousBackground)
    ...
    '--brand-font':
      data.font === 'editorial'
        ...
        : data.font === 'mono'
          ? "'DM Mono', monospace"
          : data.font === 'code-pro'
            ? "'Code Pro', 'DM Sans', Arial, sans-serif"
    ...
    >
      ...
      <header className="link-tree-header">
        <div className="link-tree-header-actions">
          ...
          {toast && (
            <div className="link-tree-toast" role="status">
              {toast}
            </div>
          )}
        </div>
      ...
      <section
        className="link-tree-content"
        aria-labelledby="linktree-profile-name"
      >
        <div className="link-tree-main">
              ...
              links.map((link, index) => (
                ...
                >
                  ...
                  <span className="link-card-arrow">
                    <LinkTreeIcon name="arrow" size={20} />
                  </span>
                </a>
              ))
            ) : (
              <div className="link-tree-empty">
                Agregá tu primer link para verlo acá.
              </div>
            )}
          </div>

          {(data.locationUrl || data.websiteUrl) && (
              ...
              {data.locationUrl && (
                ...
                >
                  <span className="feature-icon-circle">
                    <LinkTreeIcon name="map" size={17} />
                  </span>
                  <span className="feature-eyebrow">Encontranos</span>
                  <span className="feature-title">Ver ubicación</span>
                  <span className="feature-arrow">
                    <LinkTreeIcon name="arrow" size={14} />
                  </span>
                </a>
              )}
              {data.websiteUrl && (
                <a
                  className="feature-card feature-card--website"
                  href={data.websiteUrl}
                  target="_blank"
                  rel="noreferrer"
                >
                  <span className="feature-icon-circle">
                    <LinkTreeIcon name="link" size={17} />
                  </span>
          ...
          <div className="link-tree-socials">
            {data.instagramUrl && (
              ...
              >
                <LinkTreeIcon name="instagram" size={16} />
                Instagram
              </a>
            )}
            {data.tiktokUrl && (
              <a
                ...
                aria-label="TikTok"
              >
                <LinkTreeIcon name="tiktok" size={16} />
                TikTok
              </a>
            )}
            {data.emailUrl && (
              <a href={emailHref(data.emailUrl)} aria-label="Enviar email">
                <LinkTreeIcon name="mail" size={16} />
                Email
              </a>
            )}
            {data.whatsappUrl && (
              <a
                ...
                aria-label="WhatsApp"
              >
                <LinkTreeIcon name="whatsapp" size={16} />
                WhatsApp
              </a>
            )}
            {data.websiteUrl && (
        ...
        <a
          className={`link-tree-footer${darkBackground ? ' link-tree-footer--dark' : ''}`}
        ...
        >
          ...
          <span className="link-tree-footer-mark">
