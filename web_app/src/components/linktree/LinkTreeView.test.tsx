...
describe('LinkTreeView', () => {
  it('renders business-specific profile and only enabled links', () => {
    const view = render(
      <MemoryRouter>
        <LinkTreeView
          data={tree}
          preview
          publicUrl="http://localhost:3001/l/studio"
        />
      </MemoryRouter>
    )
    ...
    expect(
      view.container
        .querySelector('.link-tree-page')
        ?.classList.contains('link-tree-page--botanical')
    ).toBe(true)
  })
  ...
  it('turns an email address into a mailto link', () => {
    const view = render(
      <MemoryRouter>
        <LinkTreeView data={{ ...tree, emailUrl: 'hola@studio.test' }} />
      </MemoryRouter>
    )
    expect(view.getByLabelText('Enviar email').getAttribute('href')).toBe(
      'mailto:hola@studio.test'
    )
  })
  ...
  it('uses the selected visual template without changing the page content', () => {
      ...
      <MemoryRouter>
        <LinkTreeView
          data={{
            ...
            websiteUrl: 'https://studio.example.test',
          }}
        />
      </MemoryRouter>
    )
    ...
    expect(
      view.container
        .querySelector('.link-tree-page')
        ?.classList.contains('link-tree-page--editorial')
    ).toBe(true)
    expect(view.getByRole('heading', { name: 'Studio Objects' })).toBeTruthy()
