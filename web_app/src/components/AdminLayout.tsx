  ...
  return (
    <div className="min-h-screen bg-[var(--color-bg-primary)] transition-colors font-sans">
      ...
      <div className="lg:pl-64">
        <AdminHeader
          onMobileToggle={() => dispatch(toggleMobileMenu())}
          onDesktopToggle={() => dispatch(toggleSidebar())}
        />
...
function MobileSidebar({
  ...
  return (
    <aside
      className={`fixed inset-y-0 left-0 z-50 w-64 transform bg-[var(--color-bg-secondary)] border-r border-[var(--color-border)] transition-transform duration-300 lg:hidden ${open ? 'translate-x-0' : '-translate-x-full'}`}
    >
    ...
    <nav className={`${className} p-4 space-y-1`}>
      {navigation.map((item) => (
        <Link
          ...
          to={item.href}
          onClick={onNavigate}
          className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors ${pathname === item.href ? 'bg-[var(--color-bg-elevated)] text-[var(--color-accent)]' : 'text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-card)] hover:text-[var(--color-text-primary)]'}`}
        >
