...
import { SOCIALS, activeSocials, hrefOf, socialError } from './socials'

describe('hrefOf', () => {
  it('builds the wa.me link from the stored digits', () => {
    expect(hrefOf('socialWhatsapp', '59899123456')).toBe('https://wa.me/59899123456')
  })
...
describe('activeSocials', () => {
  it('returns only what the shop filled in, in display order', () => {
    ...
    expect(active.map((s) => s.id)).toEqual(['socialInstagram', 'socialWebsite'])
  })
...
describe('socialError', () => {
  ...
  it.each(['instagram.com/micafe', 'https://instagram.com/micafe', 'www.micafe.uy'])(
    'accepts %s',
    (value) => {
  ...
  it.each(['', '   ', '@'])('treats %p as an empty field, not an error', (value) => {
    expect(socialError('socialInstagram', value)).toBeNull()
  })
  ...
  it('wants a phone with a country code for whatsapp', () => {
    ...
    expect(socialError('socialWhatsapp', '1'.repeat(16))).toBe('social.error.phone')
  })
})
