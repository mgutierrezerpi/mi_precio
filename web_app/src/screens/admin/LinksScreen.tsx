          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Field label="Estilo">
              <select
                ...
                value={link.style}
                onChange={(event) =>
                  set('style', event.target.value as LinkTreeLinkStyle)
                }
              >
  ...
  return (
    ...
    >
      <main className="mx-auto flex min-h-full w-full max-w-[1320px] flex-col gap-5 overflow-x-hidden px-4 py-6 md:px-10 md:py-8">
        <header className="flex flex-col gap-4 border-b border-[var(--dash-border)] pb-5 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {(isDirty || isSaving) && (
              <button
                type="button"
                disabled={!tree || isSaving || !canEdit || !isDirty}
                onClick={() => void save()}
        ...
        {!isLoading && tree && (
          <div
            className={`grid min-w-0 items-start gap-5 ${isPreviewOpen ? 'xl:grid-cols-[minmax(0,1fr)_390px]' : 'xl:grid-cols-1'}`}
          >
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <Field label="Usuario o handle">
                    <input
                      ...
                      value={tree.handle || ''}
                      onChange={(event) =>
                        update('handle', event.target.value || null)
                      }
                    />
                  </Field>
                  ...
                  <Field label="Descripción">
                    <textarea
                      ...
                      value={tree.bio || ''}
                      onChange={(event) =>
                        update('bio', event.target.value || null)
                      }
                    />
                  </Field>
                  <Field label="Imagen de perfil">
                    <div className="mt-1 flex items-center gap-3 rounded-xl border border-[var(--dash-border)] bg-[var(--dash-bg)] p-3">
                      <div className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-full border border-[var(--dash-border)] bg-[var(--dash-soft)] text-xs font-bold text-[var(--dash-link)]">
                        {tree.avatarUrl || tenant?.logoUrl ? (
                          <img
                            src={tree.avatarUrl || tenant?.logoUrl || ''}
                      ...
                      <div className="flex min-w-0 flex-1 gap-2">
                        <button
                          type="button"
                          disabled={
                            !canEdit || !tenant?.logoUrl || isUploadingAvatar
                          }
                          onClick={() =>
                            void selectCompanyLogo(tenant?.logoUrl || '')
                          }
                    ...
                    <p className="mt-2 text-[11px] font-medium text-[var(--dash-muted)]">
                      Las imágenes se optimizan y guardan de forma segura. Al
                      elegir una, tomamos su color dominante para la página.
                    </p>
                  </Field>
                  ...
                  <Field label="Plantilla">
                    <select
                      ...
                      onChange={(event) =>
                        update(
                          'template',
                          event.target.value as LinkTreeTemplate
                        )
                      }
                    >
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <Field label="Instagram">
                    <input
                      ...
                      value={tree.instagramUrl || ''}
                      onChange={(event) =>
                        update('instagramUrl', event.target.value || null)
                      }
                    />
                  </Field>
                  <Field label="TikTok">
                    <input
                      ...
                      value={tree.tiktokUrl || ''}
                      onChange={(event) =>
                        update('tiktokUrl', event.target.value || null)
                      }
                    />
                  </Field>
                  <Field label="Email">
                    <input
                      ...
                      value={tree.emailUrl?.replace(/^mailto:/i, '') || ''}
                      onChange={(event) =>
                        update('emailUrl', event.target.value || null)
                      }
                    />
                  </Field>
                  <Field label="WhatsApp">
                    <input
                      ...
                      value={tree.whatsappUrl || ''}
                      onChange={(event) =>
                        update('whatsappUrl', event.target.value || null)
                      }
                    />
                  </Field>
                  <Field label="Sitio web">
                    <input
                      className={inputClass}
                      placeholder="https://…"
                      value={tree.websiteUrl || ''}
                      onChange={(event) =>
                        update('websiteUrl', event.target.value || null)
                      }
                    />
                  </Field>
                  <Field label="Ubicación">
                    <input
                      ...
                      value={tree.locationUrl || ''}
                      onChange={(event) =>
                        update('locationUrl', event.target.value || null)
                      }
                    />
                  </Field>
                </div>
            ...
            <aside
              id="links-preview"
              className={`order-first min-w-0 xl:sticky xl:top-5 xl:order-none ${isPreviewOpen ? 'max-sm:fixed max-sm:inset-0 max-sm:z-40 max-sm:overflow-y-auto max-sm:bg-[var(--dash-bg)]' : 'hidden'}`}
            >
                  ...
                  <div className="mx-auto w-full max-w-[350px] overflow-hidden rounded-[28px] border-[7px] border-[#272924] bg-[var(--dash-surface)] shadow-[0_20px_42px_rgba(25,30,23,.22)] max-sm:max-w-none max-sm:rounded-none max-sm:border-0 max-sm:shadow-none">
                    ...
                    <div>
                      <LinkTreeView
                        ...
                        preview
                        publicUrl={publicUrl}
                        fallbackAvatarUrl={tenant?.logoUrl}
                      />
                    </div>
                  </div>
                </div>
      ...
      {tree && (
        ...
        >
          ...
          <span className="[writing-mode:vertical-rl]">
            {isPreviewOpen ? 'Ocultar preview' : 'Abrir preview'}
          </span>
        </button>
      )}
    </CrmLayout>
