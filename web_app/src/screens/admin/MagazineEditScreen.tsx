...
type MagazineTextField = Exclude<
  MagazineEditField,
  'image' | 'productName' | 'productDescription' | 'productPrice'
>
...
export function MagazineEditScreen() {
  ...
  const historyGroupRef = useRef<{ pageId: string; changedAt: number } | null>(
    null
  )
  ...
  const load = useCallback(async () => {
    ...
    const orderedPages = [...found.pages].sort(
      (a, b) => a.position - b.position
    )
    const nextDrafts = Object.fromEntries(
      orderedPages.map((page) => [page.id, pageDraftFrom(page)])
    )
    setMagazine(found)
  ...
  const currentPage =
    pages[Math.min(currentIndex, Math.max(0, pages.length - 1))]
  ...
  const savePage = useCallback(
    async (pageId: string, draft: MagazinePageDraft) => {
      setSaveState('saving')
      const response = await api.updateMagazinePage(pageId, pagePayload(draft))
      if (response.data) {
        setPages((current) =>
          current.map((page) => (page.id === pageId ? response.data! : page))
        )
  ...
  const scheduleSave = useCallback(
    (pageId: string, draft: MagazinePageDraft) => {
  ...
  const updateDraft = (
    updater: (draft: MagazinePageDraft) => MagazinePageDraft
  ) => {
    ...
    if (
      !historyGroup ||
      historyGroup.pageId !== pageId ||
      now - historyGroup.changedAt > 700
    ) {
  ...
  const updateProductText = (
    field: 'productName' | 'productDescription' | 'productPrice',
    value: string
  ) => {
    const index = selection?.productIndex ?? 0
    updateDraft((draft) => {
      const defaults =
        DEFAULT_MAGAZINE_PRODUCTS[draft.layout] ??
        DEFAULT_MAGAZINE_PRODUCTS.pantry
      ...
      const products = Array.from(
        { length: productCount },
        (_, productIndex) => {
          const base = defaults[productIndex] ??
  ...
  const updateImage = (value: string) => {
    ...
    updateDraft((draft) => {
      ...
      return {
        ...
        imageUrl: compact[0] ?? '',
        images: compact,
        imagePositions: compact.map(
          (_, imageIndex) => draft.imagePositions[imageIndex] || 'center'
        ),
      }
    })
  }
  ...
  if (!canEdit)
    return (
      <div className="p-8 text-sm text-[var(--dash-muted)]">
        {t('common.error')}
      </div>
    )
  if (loading)
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#17120f] text-sm text-[#F3EDE2]">
        {t('magazines.loading')}
      </div>
    )
  if (error && !magazine)
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-[#17120f] px-6 text-center text-[#F3EDE2]">
        <p>{error}</p>
        <Link to="/admin/magazines" className="underline">
    ...
    <div className="flex min-h-screen flex-col bg-[#241B15] text-[#F3EDE2]">
        ...
        <div className="mx-auto flex w-full max-w-[1280px] items-center justify-between gap-4">
          <div className="flex min-w-0 items-center gap-3">
            <button
              ...
              className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#3A2A1D]/80 text-[#D6B58B] hover:bg-[#3A2A1D]"
              aria-label={t('magazines.backToMagazine')}
            >
              <Icon name="chevron-left" size={16} />
            </button>
            <div className="min-w-0" style={{ fontFamily: MAGAZINE_MONO }}>
              <p className="max-w-[190px] truncate text-[8px] uppercase tracking-[1.8px] text-[#D6B58B] sm:max-w-none">
                {magazine.name}
              </p>
              <p className="mt-0.5 text-[8px] uppercase tracking-[1.3px] text-[#F3EDE2]/75">
                {t('magazines.editTitle')}
              </p>
            </div>
          </div>
          <div
            className="flex min-w-0 max-w-[28vw] flex-1 items-center justify-start gap-1.5 overflow-x-auto py-1 [scrollbar-width:none] md:max-w-none md:justify-center md:overflow-visible"
            ...
            {pages.map((page, index) => (
              <button
                ...
                onClick={() => {
                  ...
                  setSelection(null)
                  historyGroupRef.current = null
                  setUndoCount(undoStacksRef.current.get(page.id)?.length ?? 0)
                }}
                className={`h-1.5 rounded-full transition-all ${index === currentIndex ? 'w-6 bg-[#D6B58B]' : 'w-1.5 bg-[#F3EDE2]/30 hover:bg-[#F3EDE2]/60'}`}
              />
            ))}
          </div>
          <div
            className="flex shrink-0 items-center gap-1.5 sm:gap-2.5"
            style={{ fontFamily: MAGAZINE_MONO }}
          >
            <span className="text-[9px] uppercase tracking-[1.3px] text-[#F3EDE2]/80">
            ...
            <button
              ...
              className="flex h-8 items-center gap-1 rounded-full border border-[#F3EDE2]/10 px-2.5 text-[8px] uppercase tracking-[.8px] text-[#D6B58B] hover:bg-[#3A2A1D] disabled:cursor-not-allowed disabled:opacity-35"
              aria-label={`${t('magazines.undo')} (⌘Z)`}
            >
              ↶<span className="hidden lg:inline">{t('magazines.undo')}</span>
              <kbd className="hidden rounded border border-[#F3EDE2]/10 px-1 py-0.5 text-[8px] normal-case tracking-normal lg:inline">
                ⌘Z
              </kbd>
            </button>
            <span
              className={`hidden text-[8px] uppercase tracking-[.9px] sm:block ${saveState === 'error' ? 'text-red-300' : saveState === 'saving' ? 'text-amber-200' : saveState === 'dirty' ? 'text-violet-200' : 'text-emerald-200'}`}
            >
            ...
            <button
              type="button"
              onClick={() => void leaveEditor()}
              className="h-8 rounded-full bg-[#D6B58B] px-3.5 text-[9px] font-bold uppercase tracking-[.9px] text-[#3A2A1D] hover:bg-[#E5BF8B]"
            >
              {t('common.done')}
            </button>
          </div>
      ...
      <div className="min-h-0 flex-1">
        <section className="relative h-full min-h-[680px]">
          {pages.length ? (
            magazine.design === 'aqua-objects' ? (
              <AquaObjectsEditorPreview
                ...
                onPageChange={(index) => {
                  ...
                  historyGroupRef.current = null
                  setUndoCount(
                    undoStacksRef.current.get(pages[index]?.id ?? '')?.length ??
                      0
                  )
                }}
                inlineEditing={selection?.field === 'image' ? null : selection}
                inlineValue={
                  selection && currentDraft
                    ? inlineTextValue(currentDraft, selection)
                    : ''
                }
                onInlineChange={(value) => {
                  if (!selection || selection.field === 'image') return
                  if (
                    selection.field === 'productName' ||
                    selection.field === 'productDescription' ||
                    selection.field === 'productPrice'
                  )
                    updateProductText(selection.field, value)
                  else updateText(selection.field as MagazineTextField, value)
                }}
                onInlineCommit={() => setSelection(null)}
              />
            ) : magazine.design === 'wild-stem' ? (
              <WildStemEditorPreview
                magazineTitle={magazine.name}
                magazinePages={renderedPages}
                pageIndex={currentIndex}
                embedded
            ...
            <div className="flex h-full min-h-[640px] items-center justify-center rounded-2xl bg-[#241b15] text-sm text-[#D6B58B]">
              {t('magazines.noPages')}
            </div>
          )}
          {currentDraft && selection?.field === 'image' && (
            <ImageControls
              draft={currentDraft}
              selection={selection}
...
function ImageControls({
  ...
  selection: MagazineEditSelection
  onImageChange: (value: string) => void
  onImagePositionChange: (value: string) => void
  onSelect: (selection: MagazineEditSelection | null) => void
  ...
  return (
    <div className="absolute bottom-5 right-5 z-40 w-[min(330px,calc(100%-2.5rem))] rounded-2xl border border-white/10 bg-[#211a16]/95 p-3 shadow-2xl backdrop-blur sm:bottom-8 sm:right-8">
      <div className="mb-3 flex items-center justify-between gap-3">
        <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#D6B58B]">
          {t('magazines.imagePreview')}
        </p>
        <button
          type="button"
          onClick={() => onSelect(null)}
          className="text-[#C8B9A9] hover:text-white"
      ...
      <div className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            {images.map((image, index) => (
              <button
                key={`${image}-${index}`}
                type="button"
                onClick={() => onSelect({ field: 'image', imageIndex: index })}
                className={`aspect-square overflow-hidden rounded-lg border-2 ${index === imageIndex ? 'border-[#D6B58B]' : 'border-white/10'}`}
              >
                <img
                  src={image}
        ...
        <label className="flex flex-col gap-1.5 text-xs font-bold text-[#D8CABB]">
          {t('magazines.imagePreview')}
          <input
            value={visibleImageUrl(images[imageIndex] ?? '')}
            onChange={(event) => onImageChange(event.target.value)}
            className={editorInput}
            placeholder="https://…"
          />
        </label>
        <label className="flex cursor-pointer items-center justify-center gap-2 rounded-lg border border-dashed border-white/15 px-3 py-2.5 text-xs font-bold text-[#D6B58B] hover:bg-white/5">
          <Icon name="upload" size={14} />
          {t('magazines.uploadImage')}
          <input
            type="file"
            accept="image/*"
            className="sr-only"
            onChange={(event) => {
              ...
              if (file) void fileToDataUrl(file, 1600).then(onImageChange)
              event.target.value = ''
            }}
          />
        </label>
        <label className="flex flex-col gap-1.5 text-xs font-bold text-[#D8CABB]">
          ...
          <select
            value={draft.imagePositions[imageIndex] || 'center'}
            onChange={(event) => onImagePositionChange(event.target.value)}
            className={editorInput}
          >
...
const editorInput =
  'w-full rounded-lg border border-white/10 bg-[#17120f] px-3 py-2.5 text-sm font-medium text-[#F3EDE2] outline-none placeholder:text-[#817466] focus:border-[#D6B58B]'
...
function pageDraftFrom(page: MagazinePage): MagazinePageDraft {
  ...
  const images = [
    ...new Set(
      [page.imageUrl ?? '', ...((content.images as string[] | undefined) ?? [])]
  ...
  return {
    ...
    title: page.title ?? String(content.headline ?? ''),
    imageUrl: images[0] ?? '',
    images,
    imagePositions: images.map((_, index) =>
      String(
        (content.imagePositions as string[] | undefined)?.[index] ?? 'center'
      )
    ),
    eyebrow: String(content.eyebrow ?? ''),
    headline: String(content.headline ?? page.title ?? ''),
    body,
    quote: String(content.quote ?? ''),
    footer: String(content.footer ?? ''),
    copy: body,
    products: Array.isArray(content.products)
      ? (content.products as MagazineProductContent[])
      : undefined,
  }
}
