...
const catColor = (c: Category): Tone =>
  COLOR_OPTIONS.includes(c.color as Tone) ? (c.color as Tone) : catTone(c.name)
...
export function CategoriesScreen() {
  ...
  const countFor = (c: Category) =>
    countByCat.m.get(c.name.trim().toLowerCase()) ?? 0
  ...
  const kpis: {
  ...
  }[] = [
    {
      ...
      label: t('categories.total'),
      note: t('categories.inCatalog'),
    },
    {
      icon: 'package',
      iconTone: 'sky',
      value: countByCat.withCat,
      label: t('categories.assigned'),
      note: t('categories.withCategory'),
    },
    {
      icon: 'alert-triangle',
      iconTone: 'amber',
      value: countByCat.without,
      label: t('categories.uncategorized'),
      note: t('categories.needsOrder'),
    },
    {
      icon: 'circle-check',
      iconTone: 'rose',
      value: topCategory ? topCategory.name : '—',
      label: t('categories.mostUsed'),
      note: t('categories.topCatalog'),
    },
  ]

  return (
    <CrmLayout
      active={t('categories.title')}
      ...
      <div className="flex flex-col gap-5 p-4 md:p-8 xl:min-w-[900px]">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {kpis.map((k) => (
            <div
              key={k.label}
              className="flex items-center gap-3.5 rounded-[18px] border border-[var(--dash-border)] bg-[var(--dash-surface)] px-5 py-[18px] shadow-[0_12px_30px_-12px_rgba(30,27,75,0.1)]"
            >
              <span
                className="flex h-12 w-12 shrink-0 items-center justify-center rounded-[14px]"
                style={tone(k.iconTone)}
              >
                <Icon name={k.icon} size={22} />
              </span>
              <div className="flex min-w-0 flex-1 flex-col">
                <div className="flex items-end gap-2">
                  <span className="truncate text-[22px] font-black leading-none text-[var(--dash-text)]">
                    {k.value}
                  </span>
                ...
                <span className="mt-1 truncate text-[11px] font-medium text-[var(--dash-muted)]">
                  {k.note}
                </span>
              </div>
            </div>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          {canEdit && (
            ...
            >
              <Icon name="plus" size={15} /> {t('categories.new')}
            </button>
          )}
        </div>
        ...
        {/* Grid */}
        {loading ? (
          <div className="flex h-48 items-center justify-center text-sm font-medium text-[var(--dash-muted)]">
          ...
          <div className="flex h-56 flex-col items-center justify-center gap-3 rounded-3xl border border-[var(--dash-border)] bg-[var(--dash-surface)] text-center">
            ...
            {categories.length === 0 && canEdit && (
              ...
              >
                <Icon name="plus" size={16} /> {t('categories.createFirst')}
              </button>
            )}
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map((c) => {
              const categoryTone = catColor(c)
              const n = countFor(c)
              return (
                <div
                  key={c.id}
                  className="flex flex-col gap-3.5 rounded-[20px] border border-[var(--dash-border)] bg-[var(--dash-surface)] p-5 shadow-[0_18px_50px_-22px_rgba(30,27,75,0.2)]"
                >
                  <div className="flex items-start justify-between">
                    <span
                      className="flex h-11 w-11 items-center justify-center rounded-xl"
                      style={tone(categoryTone)}
                    >
                      <Icon name={catIcon(c.name)} size={22} />
                    </span>
                    {canEdit && (
                      <CardMenu
                  ...
                  <div className="flex flex-col gap-1">
                    <h4 className="text-[18px] font-extrabold text-[var(--dash-text)]">
                      {c.name}
                    </h4>
                    <p className="line-clamp-1 text-xs font-medium text-[var(--dash-muted)]">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      onClick={() =>
                        navigate(
                          `/admin/items?cat=${encodeURIComponent(c.name)}`
                        )
                      }
...
function CardMenu({
  ...
  const toggle = () => {
    ...
    const top =
      r.bottom + 6 + 96 > window.innerHeight ? r.top - 96 - 6 : r.bottom + 6
      ...
      {open &&
        createPortal(
          ...
          >
            ...
            >
              <Icon name="settings" size={15} /> {t('categories.edit')}
            </button>
            <button
              type="button"
              onClick={() => {
            ...
            >
              <Icon name="circle-x" size={15} /> {t('categories.delete')}
            </button>
          </div>,
          document.body
        )}
...
function CategoryModal({
  ...
  const [color, setColor] = useState<Tone>(
    catColor(category ?? ({ name: '', color: null } as Category))
  )

  useModalDismiss(onClose)
  const submit = useCategorySubmit({
    category,
    color,
    description,
    dispatch,
  ...
  return (
    ...
    >
      <form
        onSubmit={submit}
        className="dash w-full max-w-[440px] animate-scale-in rounded-3xl border border-[var(--dash-border)] bg-[var(--dash-surface)] p-6 font-sans shadow-[0_30px_80px_-20px_rgba(15,23,42,0.5)]"
      >
        <CategoryModalHeader category={category} onClose={onClose} />
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-4">
            <span
              ...
              style={tone(color)}
            >
              <Icon name={catIcon(name)} size={26} />
            </span>
