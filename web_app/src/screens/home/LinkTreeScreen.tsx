...
const profileLinks = {
  ...
  booking:
    'https://calendar.google.com/calendar/r/eventedit?text=Visita%20al%20taller%20Casa%20Nativa&location=Montevideo',
  whatsapp:
    'https://wa.me/59800000000?text=Hola%20Casa%20Nativa%2C%20quiero%20consultar%20por%20una%20visita.',
  ...
  return (
    <main className="link-tree-page">
      ...
      <section className="link-tree-content" aria-labelledby="profile-name">
        ...
        <div className="link-tree-links">
          {links.map((link, index) => (
            <button
              ...
              style={
                { '--link-delay': `${index * 70}ms` } as React.CSSProperties
              }
              onClick={() => {
                if (link.action === 'catalog') setIsCatalogOpen(true)
                else if (link.action === 'booking')
                  window.open(
        ...
        <div className="link-tree-feature-grid">
          ...
          >
            <span className="feature-eyebrow">
              Diario de taller <Icon name="arrow" size={13} />
            </span>
        ...
        <p className="link-tree-footer">
          Un link. Todo tu negocio. <span>✳</span>
        </p>
      </section>
        ...
        >
          ...
          >
            ...
            <div className="catalog-modal-heading">
              ...
              <h2 id="catalog-title">
                Catálogo <em>de invierno.</em>
              </h2>
              <p>
                Precios actualizados · Retiro en taller o envíos a todo el país
              </p>
            </div>
            <div className="catalog-items">
              {priceItems.map(([name, price], index) => (
                <div className="catalog-item" key={name}>
                  <span
                    className={`catalog-item-art catalog-item-art--${index + 1}`}
                    aria-hidden="true"
                  />
                  <span>
                    <strong>{name}</strong>
                    <small>Edición limitada · hecho a mano</small>
                  </span>
                  <b>{price}</b>
                </div>
              ))}
