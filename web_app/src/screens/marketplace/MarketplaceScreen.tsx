...
function BusinessCard({ business }: { business: MarketplaceBusiness }) {
  ...
  return (
    <div className="group flex min-h-52 flex-col rounded-xl border border-[var(--dash-border)] bg-[var(--dash-surface)] p-4 transition hover:border-[var(--dash-link)] hover:shadow-[0_12px_28px_-18px_rgba(15,23,42,0.45)]">
      <div className="flex items-start justify-between gap-3">
        ...
        {distance && (
          <span className="shrink-0 rounded-full px-2.5 py-1 text-[11px] font-bold" style={tone('green')}>
            {distance}
          </span>
        )}
      </div>
      ...
      {business.address && (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="mt-3 inline-flex min-w-0 items-center gap-1 truncate text-xs font-semibold text-[var(--dash-muted)] hover:text-[var(--dash-link)]"
        >
      ...
      {(business.whatsappUrl || business.websiteUrl || business.instagramUrl) && (
        <div className="mt-3 flex flex-wrap gap-2">
          {business.whatsappUrl && <ContactLink href={business.whatsappUrl} label="WhatsApp" />}
          {business.websiteUrl && <ContactLink href={business.websiteUrl} label="Web" />}
          {business.instagramUrl && <ContactLink href={business.instagramUrl} label="Instagram" />}
        </div>
      )}
...
function ContactLink({ href, label }: { href: string; label: string }) {
  return (
    <a
      ...
      className="inline-flex items-center gap-1 rounded-full border border-[var(--dash-border)] bg-[var(--dash-soft)] px-2.5 py-1 text-[11px] font-bold text-[var(--dash-link)] hover:border-[var(--dash-link)]"
    >
