...
import { generateQrMatrix } from '../../src/lib/qrcode'
...
function alignPositions(version: number): number[] {
  ...
  const res = [6]
  for (let pos = version * 4 + 10; res.length < n; pos -= step)
    res.splice(1, 0, pos)
  return res
}
  ...
  for (let i = 0; i < ap.length; i++)
    for (let j = 0; j < ap.length; j++) {
      ...
      for (let dy = -2; dy <= 2; dy++)
        for (let dx = -2; dx <= 2; dx++) mark(ap[i] + dx, ap[j] + dy)
    }
  // Format areas.
  for (let i = 0; i <= 8; i++) {
    mark(8, i)
...
function decode(text: string, ecc: 'L' | 'M' | 'Q' | 'H'): string {
  ...
  for (const byte of dataBytes)
    for (let k = 7; k >= 0; k--) bitstream.push((byte >> k) & 1)
  let p = 0
  const take = (n: number) => {
    let v = 0
    for (let k = 0; k < n; k++) v = (v << 1) | bitstream[p++]
    return v
  }
  const mode = take(4)
